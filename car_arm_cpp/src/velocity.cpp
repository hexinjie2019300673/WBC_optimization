...
// 这个函数的目标 不是去 寻点 而是进行轨迹跟踪，所以要求末端的精度
Eigen::Matrix<double, 19, 1> Car_arm::Solver_double_velocity()
{
    // 解决问题 car + left arm
...
void Car_arm::Update_parameter_double_velocity(const Eigen::Matrix<double, 4, 4> &T_World_Carbase_input,
                                               ...
                                               const Eigen::Matrix<double, 7, 1> &q_right_input)
{
    ...
    T_Base_Leffector = Fkine(dh_list_Base_Leffector, q_Base_Leffector);
    T_Base_Reffector = Fkine(dh_list_Base_Reffector, q_Base_Reffector);
    Eigen::Matrix<double, 4, 4> T_Carbase_Leffector = T_Carbase_Base * T_Base_Leffector;
