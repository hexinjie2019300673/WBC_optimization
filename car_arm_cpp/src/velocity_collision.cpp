...
Eigen::Matrix<double, 19, 1> Car_arm::Solver_collision()
{
    // 解决问题 car + left arm
    ...
    Eigen::Matrix<double, 31, 1> C_collision_self, C_collision_outer;
    ...
    C = C + C_collision_outer + C_collision_self;

    //  -------------------J---------------------------------

    Matrix<double, 3, 3>
        R_Carbase_base = T_Carbase_Base.block(0, 0, 3, 3);
...
void Car_arm::Update_parameter_collision(const Eigen::Matrix<double, 4, 4> &T_World_Carbase_input,
                                         ...
                                         const Eigen::Matrix<double, 7, 1> &q_right_input)
{
    ...
    T_Base_Leffector = Fkine_collision(dh_list_Base_Leffector, q_Base_Leffector, 1);
    T_Base_Reffector = Fkine_collision(dh_list_Base_Reffector, q_Base_Reffector, 0);
    Eigen::Matrix<double, 4, 4> T_Carbase_Leffector = T_Carbase_Base * T_Base_Leffector;
